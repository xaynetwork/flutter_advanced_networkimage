import 'dart:ui' as ui show Image, ImageFilter, OffsetBase;
...
void myPaintImage({
  ...
  ui.OffsetBase outputSize = rect.size,
      inputSize = Size(image.width.toDouble(), image.height.toDouble());
  late Offset sliceBorder;
  if (centerSlice != null) {
    sliceBorder = Offset(
      centerSlice.left + (inputSize as Size).width - centerSlice.right,
      centerSlice.top + inputSize.height - centerSlice.bottom,
    );
    outputSize = (outputSize as Size) - sliceBorder;
    inputSize -= sliceBorder;
  }
  fit ??= centerSlice == null ? BoxFit.scaleDown : BoxFit.fill;
  assert(centerSlice == null || (fit != BoxFit.none && fit != BoxFit.cover));
  final FittedSizes fittedSizes =
      applyBoxFit(fit, (inputSize as Size) / scale, outputSize as Size);
